%from http://www.romps.berkeley.edu/papers/pubs-2016-lcl.html
function z = lclcal(p,T,rhl,return_ldl,return_min_lcl_ldl)
   ...
   lcl = cpm*T/ggr*( 1 - bl/(al*wapr(bl/al*cl^(1/al),-1)) );
   ldl = cpm*T/ggr*( 1 - bs/(as*wapr(bs/as*cs^(1/as),-1)) );

   % Return either lcl or ldl
   if return_ldl & return_min_lcl_ldl
      error('Error in lcl: return_ldl and return_min_lcl_ldl can not both be true')
   elseif return_ldl
      z = ldl;
   elseif return_min_lcl_ldl
      if lcl_liquid < lcl_solid
         z = lcl_liquid;
      else
         z = lcl_solid;
      end
...
%
% D. A. Barry, First version submitted to Mathworks on 23 June 2003
% Updated to remove warnings in the code, 31 March 2016
% andrew.barry@epfl.ch
%
